    ...
    ) : ISyncService
{
    public async Task SyncCitiesAndMonumentsIfNeededAsync(CancellationToken cancellationToken = default)
    {
        ...
        {
            var city = await GetCityByRemoteIdAsync(db, cityManifest.RemoteId, cancellationToken);
            if (city is null)
                continue;

            var cityManifestDetails = await GetCityManifestAsync(cityManifest.ManifestPath, cancellationToken);
            ...
            {
                await SyncMonumentsAsync(db, newMonuments, cancellationToken);
            }
        }
    ...
    private Task<CityDetailsManifest?> GetCityManifestAsync(
        string manifestPath,
        CancellationToken cancellationToken)
    {
        return _manifestClient.GetCityManifestAsync(manifestPath, cancellationToken);
    }

    internal async Task<List<City>> GetNewCitiesAsync(
       KnowTheCityDbContext db,
    ...
    private static Task<City?> GetCityByRemoteIdAsync(
        KnowTheCityDbContext db,
        string remoteId,
        CancellationToken cancellationToken)
    {
        return db.Cities
            .FirstOrDefaultAsync(x => x.RemoteId == remoteId, cancellationToken);
    }

    internal async Task<List<Landmark>> GetNewMonumentsAsync(
        KnowTheCityDbContext db,
        int cityId,
